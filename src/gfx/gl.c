...
CanimGfxDevice *gl_create_device(CanimLogger *c_log,
    ...
    if (dev->headless) {
        ...
        err = CGLCreatePBuffer(info->width, info->height, GL_TEXTURE_2D,
                               GL_RGBA, 0, &pbuffer);
        if (err != kCGLNoError) {
            const char *errStr = CGLErrorString(err);
        ...
        EGLConfig cfg;
        EGLint N;
        if (!eglChooseConfig(dev->egl_display, cfg_attr, &cfg, 1, &N) ||
            N == 0) {
        ...
        EGLint pb_attr[] = {EGL_WIDTH, info->width, EGL_HEIGHT, info->height,
                            EGL_NONE};
        dev->egl_surface =
            eglCreatePbufferSurface(dev->egl_display, cfg, pb_attr);
        ...
        if (!eglMakeCurrent(dev->egl_display, dev->egl_surface,
                            dev->egl_surface, dev->egl_context)) {
        ...
        if (SDL_Init(SDL_INIT_VIDEO) < 0) {
            ...
            CANIM_LOG_ERROR("Initializing SDL_video failed: %s",
                            SDL_GetError());
            free(dev);
            return NULL;
        }
